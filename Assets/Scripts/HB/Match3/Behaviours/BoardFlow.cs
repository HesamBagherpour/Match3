    ...
    public class BoardFlow
    {
        ...
        private void FillFlowCellsTillBlocker(Flow flow, Point pos)
        {
            ...
            for (int y = pos.y - 1; y >= 0; y--)
            {
                ...
                {
                    Cell topCell = _board.Cells[topPos.x, topPos.y];
                ...
                if (cell.IsVisible == false && cell.Contains<InvisibleBlocker>())
                {
                    ...
                    if (_board.IsInBounds(topPos))
                    {
                        ...
                        if (topCell.IsVisible && topCell.IsLocked(ActionType.Move, Direction.Center) == false)
                        {
                            ...
                            {
                                Cell leftCell = _board.Cells[leftPos.x, leftPos.y];
                            ...
                            {
                                Cell rightCell = _board.Cells[rightPos.x, rightPos.y];
        ...
        private void SetChildrenAtHeight(int _height, Flow mainFlow)
        {
            ...
            Cell cell = mainFlow.Cells[startIndex];
            if (cell.IsVisible)
            {
        ...
        private void SetChildrenFlow(Cell origin)
        {
            Point pos = origin.position;
            if (_board.IsInBounds(pos.x - 1, pos.y - 1))
            {
                Cell bottomLeft = _board.Cells[pos.x - 1, pos.y - 1];
                SetDownNeighborProvider(bottomLeft, origin);
            }
            if (_board.IsInBounds(pos.x + 1, pos.y - 1))
            {
                Cell bottomRight = _board.Cells[pos.x + 1, pos.y - 1];
                SetDownNeighborProvider(bottomRight, origin);
            }
        }

        private void SetDownNeighborProvider(Cell orphanCell, Cell origin)
        {
        ...
        private bool CanTradeBlock(Cell provider, Cell receiver)
        {
        ...
        private bool CanTradeFromTopRight(Cell topRight, Cell receiver)
        {
        ...
        private bool CanTradeFromTopLeft(Cell topLeft, Cell receiver)
        {
            // if cells are locked with any kinds of restrictions like wood or grass or iron
        ...
        private bool CanTradeFromTop(Cell provider, Cell receiver)
        {
